...
describe('ArmorPriorityQueue', () => {
	...
	let items = [90, 70, 50, 30, 10, 80, 60, 40, 20];
	let comparator = (nodeOneIndex: number, nodeTwoIndex: number) => {
	...
	describe('constructor', () => {
		it.only('should initialize empty priority queue', () => {
			const custom = new ArmorPriorityQueue<number>();
			expect(custom.size()).toBe(0);
		});

		it('should initialize priority queue with arguments.elements', () => {
			const contents = [
			...
			const custom = new ArmorPriorityQueue<number>({elements: contents});
		...
		it('should throw when arguments.elements is not an array', () => {
			...
			datatypes.forEach((data) => {
				expect(() => {
					const custom = new ArmorPriorityQueue<number>({elements: data as any});
				}).toThrow();
			});
		});
	});
